...
tries to unilaterally publish an outdated state:

- while closing a channel unilaterally
- when forcing progress
- or during slashing
...
Since disputes can themselves be disputed, we could end up in situations, where
a malicious party progresses rounds rapidly depriving the other party of the
ability to dispute. To prevent this situation we can either:
...
We elect to use the second strategy because it allows faster progress in the
case of a peer that disappeared while still guaranteeing safety.
...
`channel_slash` transaction. This transaction needs to include a state with a
higher sequence number signed by all peers.
